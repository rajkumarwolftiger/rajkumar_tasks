...
def download_audio(url):
    ...
    print(f"[INFO] Saved audio: {title}.wav")
    return title
...
# ---------------------------
# ---------------------------
# DOWNLOAD TRANSCRIPTS (Google Drive Fix)
# ---------------------------
def download_transcripts(titles_and_links):
    ...
    for title, url in titles_and_links:
        ...
        try:
            ...
            # If Google returns an HTML confirmation page
...
def show_summary():
    ...
    print("\n[SUMMARY]")
    print(f" - Audio (.wav) files: {wav_count}")
    print(f" - Transcript (.pdf) files: {pdf_count}")

    if wav_count != pdf_count:
